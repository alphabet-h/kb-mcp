...
### ~~Frontmatter スキーマ検証~~ ✅ 実装済 (feature 17)
- 実装コミット: `docs/plans/feature-17-frontmatter-schema.md` 参照
- 独自 TOML DSL + `kb-mcp validate` CLI、基本的なバリデーション (required / type
  / pattern / enum / length) のみ
- 拡張 (別 feature で): EXT-1 glob 別ルール / EXT-2 index 統合 / EXT-3 watcher
  連携 / EXT-4 MCP ツール / スキーマ推論
...
| ~~フロントマター スキーマ検証~~ ✅ 実装済 (feature 17) | 高 | 低〜中 | ✅ | 完了 |
