...
- `src/mmr.rs` — MMR 多様性再ランク (`mmr_select`、v0.7.0+)
- `src/parent.rs` — Parent retriever 表示時 content 展開 (`apply_parent_retriever`、v0.7.0+)
...
- `src/eval.rs` — `kb-mcp eval` 用の retrieval 品質評価 (任意)
- `src/config.rs` — `kb-mcp.toml` 4 階層探索 + CLI 引数とのマージ
